...
class Search:
    ...
    def bfs(initial, final):
        ...
        while queue:
            ...
            for i in queue[0][0].getNeighbors():## in each neighbor there is a list with the node itself and the distance
                if not Data.isInList(traveled,i[0]):##tests if the neighbor isnt in the traveled or in the queue before adding it to the queue
                    if not Data.isInList(queue,i[0]):## if it isnt in the next one to be traveled then you can add it in the queue
    ...
    def djikstra(initial, final):
        ...
        while queue:
            if not Data.isInList(traveled,queue[0][0]):##tests again if the node to be explored is not already in the traveled list, because i could not be in the list when it was put in the queue but could be when in the traveled list when got to be explored
                    traveled.append(queue[0][:]) ##travel the first node in the queue
                    if queue[0][0] == final:##if the node we are exploring is the one we are looking for then just stop and return the search data
                        return traveled[-1]
                    ...
                    for i in exploring[0].getNeighbors():## in each neighbor there is a list with the node itself and the distance
                        if not Data.isInList(traveled,i[0]):##test if the neighbor to be added isnt in the traveled list,if it is then its the best possible path to it was already been found and no need to search to a better one
